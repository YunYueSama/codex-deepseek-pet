...
**有自己的小动作。** 眨眼、走路、挥手、跳舞、偷吃 token、蜷起来睡觉，还有鼓腮、思考和小得意。整套角色按最新标准重新绘制：走路 32 帧，进食、挥手、跳跃、跳舞、偷吃和入睡各 16 帧，并增加完整转身原画。
...
当前分支是 **3.1 重绘预览版**，适用于 Windows x64。恢复完整角色的 2D 序列动画，不使用拆分骨骼或 3D 模型。

**已发布版本可从 Releases 下载，双击 EXE 运行，不需要 Node.js。** 当前分支的预览改动与历史安装包可能不同，请核对版本说明。
...
这次从默认立绘到动作、表情和图标，都围绕同一张标准图重画：蓝紫发色、蓝眼睛、深蓝白金裙装和一条鲸尾。散步先转身再迈步，结束时收步回正；睡醒先起身。身体调整避开最近两种动作，饥饿、哼歌则看状态，专注时保持克制。

当前以 60 FPS 为渲染目标。她采用完整角色序列帧，部分情绪仍采用关键姿势；60 FPS 不代表每秒 60 张独立原画。方向由绘制的原画决定，不是任意角度的三维旋转。

聊天回复逐段显示，点击停止会保留已经看到的文字。动画只绘制一层完整角色，避免前后姿势叠加形成双轮廓。
