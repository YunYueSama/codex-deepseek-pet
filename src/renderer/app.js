...
async function loadAssets(){for(const name of ['actions','expressions','motion-basic','motion-extra','idle-front','motion-walk','motion-idle','motion-turn'])await new Promise((resolve,reject)=>{
...
function paint(p,alpha=1){
 ...
 // 每一帧都是完整角色，头、躯干和四肢一起活动；眨眼也不叠加局部眼片。
 sprite(p.blink>.35?1:p.frame);
 ctx.restore();
}
function draw(now){
...
function updateHit(){
 ...
 if(lastPose){const p=lastPose,dx=x-300-p.x,dy=y-560-p.y,c=Math.cos(p.rotation),s=Math.sin(p.rotation);
  ...
  if(sx>=0&&sx<600&&sy>=0&&sy<600){const img=images[p.atlas],cell=img.width/p.columns;
   const hitFrame=p.blink>.35?1:p.frame;
