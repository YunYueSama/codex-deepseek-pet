...
(async()=>{
 ...
 const closedEyes=await sharp(path.join(out,'idle-front.png')).extract({left:512+184,top:194,width:132,height:49}).png().toBuffer();
 const frontBlink=await sharp(front).composite([{input:closedEyes,left:184,top:194}]).png().toBuffer();
